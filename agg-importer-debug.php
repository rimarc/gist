...
add_shortcode('agg_importer_list', function($atts){
   ...
   $args = [
        ...
        if (!empty($atts['search'])) {
            error_log('AGG Debug: Búsqueda detectada: ' . $atts['search']);
            error_log('AGG Debug: Args modificados: ' . print_r($args, true));
        }
...
/*parche responsive*/
/* Ajustar catálogo a la largura del WordPress */
.agg-catalogo-grid {
  width: 100% !important;
...
.agg-catalogo-card {
  flex: 1 1 200px; /* mínimo 200px y se adapta */
  max-width: 333px; 
  margin: 10px;
  box-sizing: border-box;
}
...
/* Contenido del lightbox centrado */
.agg-lightbox__content {
  ...
  align-items: center;
  justify-content: center;
  position: relative; /* importante para centrar internamente */
}
...
/* Botón de cierre del lightbox (X) */
.agg-lightbox__close {
  position: fixed;  /* antes absolute, ahora fixed para que no se “mueva” */
  top: 15px;        /* le damos aire respecto a la barra superior */
  right: 15px;      /* aire respecto al borde */
  font-size: 28px;
  color: #fff;
  cursor: pointer;
  z-index: 100000;
...
/* Ajuste responsive en mobile */
@media (max-width: 600px) {
  .agg-lightbox__content {
    max-width: 95vw;   /* casi todo el ancho */
    max-height: 85vh;  /* casi todo el alto */
  }

  .agg-lightbox__close {
    top: 20px;    /* un poco más bajo aún */
    right: 20px;
    font-size: 32px;
  }
}
        ...
        while ($query->have_posts()) {
           ...
           <?php
// Carrusel por tarjeta: priorizar imagen destacada chatGPT
$post_id = get_the_ID();
$media_items = [];
$thumb_items = []; // 👉 NUEVO array para miniaturas

// 1. Imagen destacada (SIEMPRE al inicio del carrusel)
...
if ($thumbnail_id) {
    ...
    $thumb_items[] = [
        ...
        'html'=>wp_get_attachment_image(
            ...
            'medium',
            false,
            ['loading'=>'lazy','decoding'=>'async','class'=>'thumb-only']
        )
    ];
}

// 2. Adjuntos (imágenes y videos) - excluye la destacada para no duplicar
...
foreach ($attachments as $att) {
    ...
    if (strpos($mime, 'image/') === 0) {
        ...
        $media_items[] = [
            ...
            'html'=>wp_get_attachment_image(
                $att->ID,
                'full', // 👉 cambiado de 'medium' a 'full'
                false,
                ['loading'=>'lazy','decoding'=>'async']
            )
        ];
        ...
        if ($src) {
            $media_items[] = [
                'type'=>'video',
                'html'=>wp_video_shortcode(['src'=>$src,'preload'=>'metadata'])
            ];
            // 👇 en este caso no hay thumb, pero podrías poner un poster si quieres
        }
    }
}

// 3. URLs externas (imagen / imagen_url) además de adjuntos
$meta = get_post_meta($post_id);
$raw = '';
if (!empty($meta['imagen'][0])) { 
    $raw = $meta['imagen'][0]; 
} elseif (!empty($meta['imagen_url'][0])) { 
    $raw = $meta['imagen_url'][0]; 
}
...
if (!empty($raw)) {
    ...
    foreach ($urls as $u) {
        ...
        if (preg_match('/\.(mp4|webm|ogg)(\?.*)?$/', $lower)) {
            $media_items[] = [
                'type'=>'video',
                'html'=>wp_video_shortcode(['src'=>$u,'preload'=>'metadata'])
            ];
        } else {
            // Imagen grande
            $media_items[] = [
                'type'=>'image',
                'html'=>'<img src="'.esc_url($u).'" loading="lazy" decoding="async" alt="">'
            ];
            // Miniatura
            $thumb_items[] = [
                'type'=>'image',
                'html'=>'<img src="'.esc_url($u).'" loading="lazy" decoding="async" alt="" class="thumb-only">'
            ];
        }
    }
}

// 👉 Deduplicar por HTML (solo en media_items, no en thumbs)
$seen = [];
...
(function(){
    ...
    // 🔹 Ocultar imágenes hasta que carguen
        ...
        if (img.complete) {
            ...
            img.addEventListener('load', () => {
                ...
                if (loadedCount === imgs.length) {
                    c.classList.add('is-loaded'); // mostrar carrusel cuando todas estén listas
                }
            });
        }
    });

    // 🔹 Si ya estaban todas cargadas al inicio
    if (loadedCount === imgs.length) {
...
// LÍNEA 1133 - ANTES del }); final, agrega todo este código:

/**
 * INTEGRACIÓN CON BÚSQUEDA DE HESTIA
 * Agregar ANTES del }); final
 */

// 1) Interceptar búsqueda de Hestia
...
// 2) Crear página virtual para resultados
add_action('init', function() {
    ...
    add_action('template_redirect', function() {
        if (get_query_var('agg_search_results')) {
                    ...
                    <div class="col-md-12">
                        <div class="agg-search-results">
                            ...
                            if (!empty($search_query)) {
                                // Debug: verificar que el shortcode se ejecute
                                $shortcode_output = do_shortcode('[agg_importer_list count="20" search="' . esc_attr($search_query) . '"]');
                                if (!empty($shortcode_output)) {
                                    echo $shortcode_output;
                                } else {
                                    echo '<p>No se encontraron productos con el carrusel personalizado.</p>';
                                    // Fallback: mostrar resultados básicos
                                    ...
                                    if ($fallback_query->have_posts()) {
                                        echo '<div class="agg-catalogo-grid">';
                                        while ($fallback_query->have_posts()) {
                                            $fallback_query->the_post();
                                            echo '<div class="agg-catalogo-card">';
                                            echo '<h3>' . get_the_title() . '</h3>';
                                            echo '<p>' . wp_strip_all_tags(get_the_content()) . '</p>';
                                            $price = get_post_meta(get_the_ID(), 'precio', true);
                                            if ($price) echo '<p><strong>Precio: R$ ' . esc_html($price) . '</strong></p>';
                                            echo '</div>';
                                        }
                                        echo '</div>';
                                        wp_reset_postdata();
            ...
            <style>
            .agg-search-results { padding: 40px 0; text-align: center; }
            .agg-search-results h1 { color: #333; margin-bottom: 10px; }
            .agg-search-results p { color: #666; margin-bottom: 30px; }
            .agg-search-back { margin-top: 40px; }
            .agg-search-back .btn { padding: 12px 24px; font-size: 16px; border-radius: 6px; }
            </style>
            <?php get_footer();
            exit;
        }
...
// 3) Modificar shortcode para soportar búsqueda
...
// 4) Flush rewrite rules al activar
