...
add_shortcode('agg_importer_list', function($atts){
	...
	if ($query->have_posts()) {
		...
		<style>
			...
			/* Compactar y controlar espacios */
			.agg-catalogo-grid {
				display: flex;
				flex-wrap: wrap;
				gap: 20px;
			...
			.agg-carousel-track { position: relative; overflow: hidden; padding: 0; }
...
.agg-lightbox__content { max-width: 90vw; max-height: 90vh; }
.agg-lightbox__content img, .agg-lightbox__content video {
  max-width: 90vw; max-height: 90vh; object-fit: contain; display: block;
}
.agg-lightbox__close, .agg-lightbox__nav {
  position: fixed; top: 14px; color: #fff; background: rgba(0,0,0,.5);
  border: 0; cursor: pointer; padding: 8px 12px; border-radius: 4px; z-index: 100000;
}
...
<!-- Inserta una sola vez al final del shortcode -->
