<?php
/*
Plugin Name: AGG Importer - Hestia Integration
Description: Importa productos desde CSV y muestra catálogo con shortcode integrado con Hestia.
Version: 8.8
Author: rimarc
*/
...
/**
 * 0) Front-end: reproducir videos (MediaElement) + estilos Hestia
 */
add_action('wp_enqueue_scripts', function(){
    wp_enqueue_style('wp-mediaelement');
    wp_enqueue_script('wp-mediaelement');
    
    // Estilos específicos para Hestia
    wp_enqueue_style('agg-hestia-style', plugin_dir_url(__FILE__) . 'agg-hestia-style.css', [], '1.0');
});
...
/**
 * 8) Shortcode: lista de items con carrusel optimizado para Hestia
...
<style>
/* Estilos integrados con Hestia */
.agg-hestia-container {
    max-width: 1200px;
    margin: 0 auto;
    padding: 0 15px;
}
...
.agg-hestia-card:hover {
    transform: translateY(-5px);
    box-shadow: 0 5px 20px rgba(0,0,0,0.15);
}

.agg-hestia-card-content {
    padding: 20px;
}
...
.agg-hestia-slide.is-active {
    display: block;
}

.agg-hestia-slide img,
.agg-hestia-slide video {
    width: 100%;
    height: 200px;
    object-fit: cover;
    display: block;
...
.agg-hestia-btn--custom {
    background: #007bff;
    color: #fff;
}

.agg-hestia-btn:hover {
    opacity: 0.8;
...
.agg-hestia-lightbox.is-open {
    display: flex;
}

.agg-hestia-lightbox-content {
    max-width: 90vw;
    max-height: 90vh;
    margin: 0 auto;
}

.agg-hestia-lightbox-content img,
.agg-hestia-lightbox-content video {
    max-width: 90vw;
    max-height: 90vh;
    object-fit: contain;
    display: block;
...
.agg-hestia-lightbox-nav--prev {
    left: 20px;
}

.agg-hestia-lightbox-nav--next {
    right: 20px;
}
</style>

        <div class="agg-hestia-container">
            <div class="agg-hestia-grid">
            <?php
            while ($query->have_posts()) {
                $query->the_post();
                $meta = get_post_meta(get_the_ID());
                ?>
                <div class="agg-hestia-card">
                    <?php
                    // Carrusel optimizado para Hestia
                    $post_id = get_the_ID();
                    $media_items = [];
                    ...
                    if (!empty($media_items)) :
                        $carousel_id = 'agghestia_' . $post_id;
                    ?>
                    <div class="agg-hestia-carousel" id="<?php echo esc_attr($carousel_id); ?>">
                        <div class="agg-hestia-track">
                            <?php foreach ($media_items as $idx => $m) : ?>
                                <div class="agg-hestia-slide<?php echo $idx === 0 ? ' is-active' : ''; ?>">
                                    <?php echo $m['html']; ?>
                                </div>
                            <?php endforeach; ?>
                        </div>
                        <?php if (count($media_items) > 1) : ?>
                            <button class="agg-hestia-nav agg-hestia-prev" type="button" aria-label="Anterior">‹</button>
                            <button class="agg-hestia-nav agg-hestia-next" type="button" aria-label="Siguiente">›</button>
                            <div class="agg-hestia-dots">
                                <?php foreach ($media_items as $i => $_) : ?>
                                    <button class="agg-hestia-dot<?php echo $i===0?' is-active':''; ?>" data-index="<?php echo $i; ?>" aria-label="Ir a <?php echo $i+1; ?>"></button>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
                    </div>
                    ...
                    (function(){
                        ...
                        if(!c) return;
                        const slides = Array.from(c.querySelectorAll('.agg-hestia-slide'));
                        const prev = c.querySelector('.agg-hestia-prev');
                        const next = c.querySelector('.agg-hestia-next');
                        const dots = Array.from(c.querySelectorAll('.agg-hestia-dot'));
                    ...
                    <div class="agg-hestia-card-content">
                        ...
                        if ($ml || $olx || $sh || $whatsapp_direct || $custom_platform) {
                          echo '<div class="agg-hestia-links">';
                          if ($ml)  echo '<a class="agg-hestia-btn agg-hestia-btn--ml" href="'.esc_url($ml).'" target="_blank" rel="nofollow noopener">Ver en Mercado Livre</a>';
                          if ($olx) echo '<a class="agg-hestia-btn agg-hestia-btn--olx" href="'.esc_url($olx).'" target="_blank" rel="nofollow noopener">Ver en OLX</a>';
                          if ($sh)  echo '<a class="agg-hestia-btn agg-hestia-btn--sh" href="'.esc_url($sh).'" target="_blank" rel="nofollow noopener">Ver en Shopee</a>';
                          if ($whatsapp_direct) {
                            echo '<a class="agg-hestia-btn agg-hestia-btn--wa" href="'.esc_url($whatsapp_direct).'" target="_blank" rel="nofollow noopener">💬 WhatsApp</a>';
                          }
                          if ($custom_platform) {
                            $platform_name = !empty($custom_platform_name) ? $custom_platform_name : 'Ver en Plataforma';
                            echo '<a class="agg-hestia-btn agg-hestia-btn--custom" href="'.esc_url($custom_platform).'" target="_blank" rel="nofollow noopener">'.$platform_name.'</a>';
                          }
                          echo '</div>';
                        }
                        ?>
            ...
            echo '</div>'; // cierra .agg-hestia-grid
            ?>

            <!-- Lightbox para Hestia -->
            <div class="agg-hestia-lightbox" id="aggHestiaLightbox" aria-hidden="true">
              <button class="agg-hestia-lightbox-close" type="button" aria-label="Cerrar">✕</button>
              <button class="agg-hestia-lightbox-nav agg-hestia-lightbox-nav--prev" type="button" aria-label="Anterior">‹</button>
              <div class="agg-hestia-lightbox-content" id="aggHestiaLightboxContent"></div>
              <button class="agg-hestia-lightbox-nav agg-hestia-lightbox-nav--next" type="button" aria-label="Siguiente">›</button>
            </div>

            <script>
            // Lightbox para Hestia
            (function(){
              const overlay = document.getElementById('aggHestiaLightbox');
              const content = document.getElementById('aggHestiaLightboxContent');
              const btnClose = overlay.querySelector('.agg-hestia-lightbox-close');
              const btnPrev  = overlay.querySelector('.agg-hestia-lightbox-nav--prev');
              const btnNext  = overlay.querySelector('.agg-hestia-lightbox-nav--next');

              let items = [];
              let idx = 0;

              function buildItemsFromCard(card){
                const slides = card.querySelectorAll('.agg-hestia-slide img, .agg-hestia-slide video');
                const fallbacks = card.querySelectorAll('img:not(.agg-hestia-slide img), video:not(.agg-hestia-slide video)');
                items = [];
              ...
              document.addEventListener('click', (e)=>{
                const media = e.target.closest('.agg-hestia-card img, .agg-hestia-card video');
                if (!media) return;
                e.preventDefault();
                const card = media.closest('.agg-hestia-card');
