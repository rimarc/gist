...
add_action('save_post_agg_item', function($post_id){
  ...
  if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;
  if (!current_user_can('edit_post', $post_id)) return;
  foreach (['link_ml','link_olx','link_shopee','whatsapp_direct_link'] as $k) {
...
/**
 * 8) Actualizar imagen_url cuando se cambia la imagen destacada
 */
...
/**
 * 9) Shortcode: lista de items con carrusel (imagen destacada/adjuntos o fallback a imagen_url)
        ...
        while ($query->have_posts()) {
            ...
            if (empty($media_items)) {
                ...
                foreach ($attachments as $att) {
                    if ($att->ID == $thumbnail_id) continue; // Exclude featured image
            ...
            <?php endif; ?>
                ...
                if ($ml || $olx || $sh || $wa) {
                  ...
                  if ($wa)  echo '<a class="agg-btn agg-btn--wa" href="'.esc_url($wa).'" target="_blank" rel="nofollow noopener">💬 WhatsApp</a>';
                  echo '</div>';
                }
                ?>
